@echo off
title ELEVATE ECOM - Relatorios Shopee [Jaguar]
cd /d "%~dp0"
...
echo.
echo  =====================================================
echo    ELEVATE ECOM - RELATORIOS SHOPEE [JAGUAR]
echo  =====================================================
echo.
echo    [1] Todos os clientes Jaguar
...
set "HORA=%HORA: =0%"
set "LOG=logs\jaguar_todos_%HOJE%_%HORA%.log"
echo  Rodando para TODOS os clientes Jaguar...
echo  Log: %LOG%
echo.
...
set "HORA=%time:~0,2%%time:~3,2%"
set "HORA=%HORA: =0%"
set "LOG=logs\jaguar_%CLIENTE%_%HOJE%_%HORA%.log"
echo.
echo  Rodando para: %CLIENTE%
echo  Log: %LOG%
echo.
"C:\Users\User\AppData\Local\Python\bin\python.exe" -X utf8 main_jaguar.py --cliente "%CLIENTE%" > "%LOG%" 2>&1
set "COD=%ERRORLEVEL%"
type "%LOG%"
goto FIM

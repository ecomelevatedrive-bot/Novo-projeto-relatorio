"""
Automação de relatórios Shopee + Monday.com via Dolphin Anty — Grupo Jaguar.
Filtra perfis com a tag 'Jaguar' no Dolphin Anty.

Uso:
  python main_jaguar.py                        # todos os clientes Jaguar, últimos 7 dias
  python main_jaguar.py 2025-04-01 2025-04-30  # período personalizado
  python main_jaguar.py --cliente "TM Santos"  # somente um cliente específico
"""
...
import config
from dolphin_browser import listar_perfis_por_tag, iniciar_perfil, parar_perfil
...
def _parse_args():
    ...
    if len(datas) == 2:
        ...
        print("Uso: python main_jaguar.py [AAAA-MM-DD AAAA-MM-DD] [--cliente NOME]")
        sys.exit(1)
...
def main():
    ...
    print(f"  Grupo: {DOLPHIN_TAG}")
    print("=" * 55)
    print(f"  Período : {date_from.strftime('%d/%m/%Y')} → {date_to.strftime('%d/%m/%Y')}")
    print(f"  Fonte   : Dolphin Anty (tag: {DOLPHIN_TAG})")
    print("=" * 55)

    print(f"\n[Dolphin] Buscando perfis com tag '{DOLPHIN_TAG}'...")
    perfis = listar_perfis_por_tag(DOLPHIN_TAG)

    # Filtra apenas clientes da lista Jaguar
    nomes_jaguar = [n.lower() for n in CLIENTES_JAGUAR]
    perfis = [p for p in perfis if p["nome"].lower() in nomes_jaguar]

    if filtro_cliente:
        perfis = [p for p in perfis if p["nome"].lower() == filtro_cliente.lower()]

    if not perfis:
        print(f"\n[Aviso] Nenhum perfil encontrado com a tag '{DOLPHIN_TAG}'.")
        print(f"Adicione a tag '{DOLPHIN_TAG}' nos perfis corretos dentro do Dolphin Anty.")
        sys.exit(0)
    ...
    print()
    print("=" * 55)
    print(f"  Automação concluída! — Grupo {DOLPHIN_TAG}")
